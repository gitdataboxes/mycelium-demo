...
import { api, Attribute, OrgInfo, OrgMember, ResponderInfo } from "@/lib/api";
...
export default function OrgDetailPage() {
  ...
  const loadOrg = useCallback(async () => {
    try {
      const [orgData, memberData, respData] = await Promise.all([
        api.organizations.get(orgId),
        api.organizations.getMembers(orgId),
        api.organizations.getResponders(orgId),
      ]);
      setOrg(orgData);
  ...
  const handleAddAttribute = async (e: React.FormEvent) => {
    ...
    setAdding(true);
    try {
      await api.organizations.createAttribute(orgId, newDirection, newContent.trim());
      setNewContent("");
      loadOrg();
    } finally {
      setAdding(false);
  ...
  const handleSaveEdit = async (attrId: string) => {
    ...
    await api.organizations.updateAttribute(orgId, attrId, editContent.trim());
    setEditingId(null);
    loadOrg();
  };

  const handleDeleteAttr = async (attrId: string) => {
    await api.organizations.deleteAttribute(orgId, attrId);
    loadOrg();
  };
  ...
  const handleVouch = async (e: React.FormEvent) => {
    ...
    setVouchError("");
    try {
      await api.organizations.vouchMember(orgId, vouchNodeId.trim());
      setVouchNodeId("");
      loadOrg();
    } catch (err) {
  ...
  if (error || !org) {
    return (
      <div className="max-w-2xl mx-auto p-8">
        <Link href="/organizations" className="text-sm text-neutral-400 hover:text-white">&larr; Back</Link>
        <p className="mt-4 text-red-400">{error || "Organization not found"}</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      <div className="flex items-center justify-between mb-8">
        <Link href="/organizations" className="text-sm text-neutral-400 hover:text-white">&larr; Organizations</Link>
      </div>

      <div className="flex items-start justify-between mb-6">
        <div>
          <h2 className="text-xl font-semibold">{org.name}</h2>
          {org.description && <p className="text-sm text-neutral-400 mt-1">{org.description}</p>}
        </div>
      ...
      {user && responders.length > 0 && !responders.some(r => r.node_id === user.node_id) && (
        <button
          onClick={() => {
            ...
            router.push(`/messages/${responder.node_id}?context=${orgId}`);
          }}
      ...
      <div className="grid gap-6 mb-8">
        <div className="bg-neutral-900/60 border border-neutral-800 rounded-xl p-6">
          {renderAttributes("Outputs \u2014 what we offer", "output", org.outputs)}
        </div>
        <div className="bg-neutral-900/60 border border-neutral-800 rounded-xl p-6">
          {renderAttributes("Inputs \u2014 what we need", "input", org.inputs)}
        </div>
        ...
        {org.is_member && (
          <form onSubmit={handleAddAttribute} className="flex gap-2">
            ...
            <input
              ...
              placeholder={newDirection === "output" ? "What does the org offer?" : "What does the org need?"}
      ...
      <div className="bg-neutral-900/60 border border-neutral-800 rounded-xl p-6 mb-6">
        ...
        <ul className="space-y-2">
          {members.map((m) => (
            <li key={m.node_id} className="flex items-center justify-between text-sm">
              <span className="text-neutral-300">{m.username || m.name || m.node_id.slice(0, 8)}</span>
